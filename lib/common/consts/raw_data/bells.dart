import 'package:alarmi/features/alarm/models/bell.dart';

List<Bell> bells = [
  // 'default'
  Bell(
    id: '0',
    category: 'default',
    name: 'Test',
    path: 'assets/audios/default/test.mp3',
  ),
  Bell(
    id: '2',
    category: 'default',
    name: 'Basic introduction uk drill',
    path: 'assets/audios/default/basic_introduction_uk_drill.mp3',
  ),
  Bell(
    id: '4',
    category: 'default',
    name: 'Common goal',
    path: 'assets/audios/default/common_goal.mp3',
  ),
  Bell(
    id: '6',
    category: 'default',
    name: 'Space',
    path: 'assets/audios/default/space.mp3',
  ),

  // 'signal'
  Bell(
    id: '8',
    category: 'signal',
    name: 'clip 01 turn it up',
    path: 'assets/audios/signal/clip_01_turn_it_up.mp3',
  ),
  Bell(
    id: '9',
    category: 'signal',
    name: 'clip 04 intro',
    path: 'assets/audios/signal/clip_04_intro.mp3',
  ),
  Bell(
    id: '11',
    category: 'signal',
    name: 'clip 06 headlines',
    path: 'assets/audios/signal/clip_06_headlines.mp3',
  ),
  Bell(
    id: '13',
    category: 'signal',
    name: 'clip 09 good morning',
    path: 'assets/audios/signal/clip_09_good_morning.mp3',
  ),
  Bell(
    id: '15',
    category: 'signal',
    name: 'clip 10 drippling drops short',
    path: 'assets/audios/signal/clip_10_drippling_drops_short.mp3',
  ),

  // 'nature'
  Bell(
    id: '17',
    category: 'nature',
    name: 'Mountain path',
    path: 'assets/audios/nature/mountain_path.mp3',
  ),
  Bell(
    id: '19',
    category: 'nature',
    name: 'Nature investigation',
    path: 'assets/audios/nature/nature_investigation.mp3',
  ),
  Bell(
    id: '21',
    category: 'nature',
    name: 'Sleep of nature short pixabay',
    path: 'assets/audios/nature/sleep_of_nature_short_pixabay.mp3',
  ),
  Bell(
    id: '23',
    category: 'nature',
    name: 'Warm campfire',
    path: 'assets/audios/nature/warm_campfire.mp3',
  ),

  // 'energy'
  Bell(
    id: '25',
    category: 'energy',
    name: 'Don\'t talk',
    path: 'assets/audios/energy/dont_talk.mp3',
  ),
  Bell(
    id: '27',
    category: 'energy',
    name: 'Guitar electro sport trailer',
    path: 'assets/audios/energy/guitar_electro_sport_trailer.mp3',
  ),
  Bell(
    id: '29',
    category: 'energy',
    name: 'Movement',
    path: 'assets/audios/energy/movement.mp3',
  ),
  Bell(
    id: '30',
    category: 'energy',
    name: 'My punch',
    path: 'assets/audios/energy/my_punch.mp3',
  ),
  Bell(
    id: '32',
    category: 'energy',
    name: 'Street customs',
    path: 'assets/audios/energy/street_customs.mp3',
  ),
];

List<String> bellCategories = ['default', 'signal', 'nature', 'energy'];

List<String> bellIcons = ['🐷', '🐷', '🐷', '🐷', '🐷', '🐷', '🐷'];
